...
export default function ProviderJobDetail({ jobId }: { jobId: string }) {
  ...
  useEffect(() => {
    const channel = supabase
      .channel(`provider-messages:${jobId}`)
      .on(
        "postgres_changes",
        {
          event: "INSERT",
  ...
  const handleMarkComplete = async () => {
    ...
    setError(null);
    try {
      await confirmCompletion(jobId, "provider");
      await loadData();
    } catch (err) {
      setError(
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await sendMessage(jobId, profile.id, chatMessage);
      setChatMessage("");
      const msgs = await getMessages(jobId);
      setMessages((msgs as any as Message[]) || []);
    } catch (err) {
